  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (activeTab === 'accounts') {
        ...
        await fetchAccounts(activeWorkspace.id);
      } else {
        count = await processSignalsCsv(file, activeWorkspace.id);
        await fetchSignals(activeWorkspace.id);
      }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg relative z-10 overflow-hidden animate-[fadeIn_0.2s_ease-out]">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          ...
          <button onClick={() => !isUploading && setIsOpen(false)} className="text-gray-400 hover:text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue rounded-md">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {result ? (
            <div className="text-center py-8 animate-[fadeIn_0.3s_ease-out]">
              ...
              <button 
                ...
                className="bg-gray-900 text-white px-6 py-3 rounded-xl font-bold hover:bg-brand-blue transition-colors"
              >
                Done
              </button>
            </div>
