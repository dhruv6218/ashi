...
import {
  FileText, Copy, Edit2, Save, ChevronDown, Download,
  LayoutGrid, Clock, Plus, FileCode2, FilePen
} from 'lucide-react';
import { useWorkspace } from '../../contexts/WorkspaceContext';
import { useArtifactStore, Artifact } from '../../store/useArtifactStore';
...
export const ArtifactStudio = () => {
  ...
  const { artifacts, isLoading, fetchArtifacts, createArtifact } = useArtifactStore();
  ...
  const [versions] = useState([
    { id: 'v1', number: 1, date: 'Today', label: 'Current' },
  ]);
  ...
  }, [activeWorkspace, fetchArtifacts]);
  ...
  const handleGenerateMock = () => {
    ...
    setTimeout(async () => {
      const newArt = await createArtifact({
        ...
        type: 'prd',
        content: '# New Feature PRD\n\n## Overview\nGenerated by AI based on recent decisions.',
        author_id: 'user-1'
      });
  ...
  return (
    <AppLayout
      ...
      actions={
        <button onClick={handleGenerateMock} disabled={isGenerating} className="bg-white border border-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm font-bold hover:bg-gray-50 shadow-sm flex items-center gap-2 disabled:opacity-50">
    ...
    >
        ...
        <div className="w-full lg:w-72 shrink-0 space-y-3 max-h-[40vh] lg:max-h-none overflow-y-auto hide-scrollbar">
          ...
          {isLoading && artifacts.length === 0 ? (
            ...
            artifacts.map(art => (
              <button
                key={art.id}
                onClick={() => { setSelectedArtifact(art); setIsEditing(false); }}
            ...
            <>
              ...
              <div className="bg-gray-50 border-b border-gray-200 p-3 flex flex-wrap items-center justify-between gap-3 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  {isEditing ? (
                    ...
                    <>
                      <button onClick={handleCopy} title="Copy markdown" className="p-1.5 text-gray-500 hover:text-gray-900 bg-white border border-gray-200 rounded-lg shadow-sm"><Copy className="w-3.5 h-3.5" /></button>
                      <button title="Export PDF" className="p-1.5 text-gray-500 hover:text-gray-900 bg-white border border-gray-200 rounded-lg shadow-sm"><Download className="w-3.5 h-3.5" /></button>
                      <button onClick={() => { setEditContent(selectedArtifact.content); setIsEditing(true); }} className="text-xs font-bold bg-white border border-gray-200 text-gray-700 px-3 py-1.5 rounded-lg flex items-center gap-1 shadow-sm hover:bg-gray-50">
                        <Edit2 className="w-3 h-3" /> Edit
                      </button>
                      <button onClick={() => setIsJiraPushOpen(true)} className="text-xs font-bold bg-blue-50 text-brand-blue border border-blue-200 px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-blue-100">
                        <LayoutGrid className="w-3 h-3" /> Push to Jira
                      </button>
                    </>
                  )}
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8">
                {isEditing ? (
                  ...
                  <pre className="font-mono text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{selectedArtifact.content}</pre>
                )}
              </div>
            </>
          ) : (
      ...
      <JiraPushModal 
        ...
        defaultSummary={selectedArtifact?.title || ''}
        defaultDescription={selectedArtifact?.content || ''}
      />
    </AppLayout>
  );
};
